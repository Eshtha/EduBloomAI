...
const CLAUDE_API_KEY = "CONNECTED";
...
// 1. Reroute the Chatbot to your Python backend
...
// 2. Reroute the Paper Generator to your Python backend
...
${Object.entries(bloomDist).filter(([, v]) => v > 0).map(([k, v]) => `- ${k}: ${v}%`).join('\n')}

Generate questions distributed across these Bloom's levels.
For each question provide: question text, type (mcq/short/long/tf/fill), Bloom's level, marks, options (for MCQ/TF), and a model answer.
